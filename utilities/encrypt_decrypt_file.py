...
try:
    if sys.argv[1] == '-e':
        other_name = \
            raw_input("What is the name of the party \
            that you want to encrypt the file to? ").strip()
        a.loadState(your_name, other_name)
        a.encrypt_file(sys.argv[2])
        print 'The encrypted file is: ' + sys.argv[2] + '.asc'
    else:
        other_name = \
            raw_input("What is the name of the party that \
            you want to decrypt the file from? ").strip()
